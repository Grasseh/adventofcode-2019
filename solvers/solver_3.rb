...
module Solvers
  class Solver3
    def solve_a(input)
      ...
      passed_count = passed_count_one.merge(passed_count_two) do |k, one, two|
        one + two
      end

      collisions = passed_count.select{|k, v| v >= 2}
    ...
    def solve_b(input)
      ...
      passed_count_one.merge(passed_count_two) do |k, one, two|
        min = [one + two, min].min
      end
